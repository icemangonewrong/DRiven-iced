...
- **Zurg/Rclone Information**:  Zurg files are located in the zurg folder with the installation scripts and run in a Docker container. Rclone is installed separately as a standalone binary and managed as a systemd service (rclone-mount.service) to mount Zurg’s WebDAV endpoint at /mnt/zurg. The script skips Zurg setup if its container is running and skips rclone installation if it’s already present. 
